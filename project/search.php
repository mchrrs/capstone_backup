...
if(isset($_POST['h_search'])){
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE LOWER(address) LIKE LOWER(?) AND LOWER(type) LIKE LOWER(?) AND LOWER(offer) LIKE LOWER(?) AND price BETWEEN ? AND ? ORDER BY date DESC");
   ...
   $furnished = filter_var($furnished, FILTER_SANITIZE_STRING);
    ...
    // Use LOWER() for case-insensitive search
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE LOWER(address) LIKE LOWER(?) AND LOWER(type) LIKE LOWER(?) AND LOWER(offer) LIKE LOWER(?) AND LOWER(bhk) LIKE LOWER(?) AND LOWER(status) LIKE LOWER(?) AND LOWER(furnished) LIKE LOWER(?) AND price BETWEEN ? AND ? ORDER BY date DESC");
   $select_properties->execute(["%{$location}%", "%{$type}%", "%{$offer}%", "%{$bhk}%", "%{$status}%", "%{$furnished}%", $min, $max]);
